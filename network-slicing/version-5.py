...
def display_network_model(Coor_BS, Coor_UE, M, S, rad):
    ...
    # Plot MBS location
    circle1 = plt.Circle((Coor_BS[0][0], Coor_BS[0][1]), 500, color='b', fill=False)
...
def KKT_method(S, m, r_m, rate_UE, rate_m_min):
    ...
    s_m_nonQoS = s_m - 3;
    s_m_QoS = s_m - s_m_nonQoS;  # Number of non-QoS users
    # s_m_QoS = s_m - s_m_nonQoS # int(s_m / 2);  # Number of QoS users
    ...
    if sum_X_m_nonQoS < 0:
        print("Not statisfying assumption", error)
    ...
    x_bandwidth_each_non_QoS_user = min(0.2, sum_X_m_nonQoS / s_m_nonQoS);
...
def plot_CDF(FN_J):
    ...
    # Getting data frame
    X = range(1000)
    df = pd.DataFrame(FN_J, index=X, columns=['Proposed algorithm'])
    # # Generate a violin plot of 'hp' grouped horizontally by 'cyl'
    #
    plt.figure(8)

    # sns.violinplot(x='PP', y='PP', data=df)

    axarr = df.hist(cumulative=True, normed=1, bins=1000)
...
rate_m_min = np.array([8, 10, 6]);  # Mbps
...
FN_J = np.zeros(1)# 0 for proposal, 1 for hard network slicing
...
for k in range(1):
    ...
    X_M[:, 0] = (S/S_sum)*bandwidth/2 # Mbps
    # print(X_M)
    # print(error)
    ...
    RM[:, 0] = (S/S_sum)*bandwidth/2

    UM = np.zeros(shape=(M, iter))  # Utility value of each MVNO
    ...
    # Step 0: Updating resource allocation at each MVNO with the initialized step
    for m in range(M):
        ...
        X_M_A, RATE_A, V_m_A, X_m_user_MVNO = KKT_method(S, m, RM[m][0], rate_UE, rate_m_min)
...
# plt.subplot(2,1,2)
# sns.violinplot(x='cyl', y='hp', data=plot_FN, inner=None, color='lightgray')
